...
def load_custom_clip(model_name, img_size=224, device=None):
	...
	preprocess.transforms[0] = tf.Resize(size=img_size, interpolation=tf.InterpolationMode.BICUBIC)
	preprocess.transforms[1] = tf.CenterCrop(size=(img_size,img_size))
	...
	preprocess_label.transforms[0] = tf.Resize(size=img_size, interpolation=tf.InterpolationMode.NEAREST)
